...
if (iconMenu != null) {
	let delay = 500;
	let menuBody = document.querySelector(".nav-menu");
...
let getInItem = document.querySelectorAll(".get-in__column");
getInItem.forEach(function(item){
	item.addEventListener('mouseover', function(){
		let nowColumn = item;
		
		if(!nowColumn.classList.contains('_active')){
			nowColumn.classList.add('_active');
		}
	});
});
getInItem.forEach(function(item){
	item.addEventListener('mouseout', function(){
		...
		if(nowColumn.classList.contains('_active')){
			nowColumn.classList.remove('_active');
		}
	});
});
